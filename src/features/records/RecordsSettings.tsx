...
const LOOKUP_CATEGORIES: { type: PersonLookupType; label: string; description: string }[] = [
  { type: 'PersonType', label: 'Person Types', description: 'Classifications such as Student, Parent, Employee.' },
  { type: 'RelationshipType', label: 'Relationship Types', description: 'Mother, Father, Guardian, Sibling, etc.' },
  { type: 'ContactType', label: 'Contact Types', description: 'Mobile, Landline, Email, Social, etc.' },
  { type: 'SectorType', label: 'Sector Types', description: 'PWD, Solo Parent, Indigenous, etc.' },
  { type: 'EducationLevel', label: 'Education Levels', description: 'Pre-Elementary through College.' },
]

export function RecordsSettings() {
  const { addToast } = useToast()
  const [activeType, setActiveType] = useState<PersonLookupType>('PersonType')
  ...
  const activeCategory = LOOKUP_CATEGORIES.find((c) => c.type === activeType)!

  const load = useCallback(async () => {
    setIsLoading(true)
    setLoadError(null)
    try {
      const data = await personLookupsApi.list(activeType)
      setItems(data)
    } catch (error) {
      const message =
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.editing) {
        await personLookupsApi.update(activeType, modal.editing.id, form.name.trim(), form.code.trim() || null)
        addToast('success', 'Entry updated', '')
      } else {
        await personLookupsApi.create(activeType, form.name.trim(), form.code.trim() || null)
  ...
  const handleDelete = async (item: LookupItem) => {
    try {
      await personLookupsApi.remove(activeType, item.id)
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[220px_1fr]">
      <Card className="!p-2 h-fit">
        <nav className="space-y-1" aria-label="Lookup table categories">
      ...
      <Card className="overflow-hidden !p-0">
        <div className="flex flex-col gap-3 border-b border-gh-border bg-gh-canvas-subtle/50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gh-fg">{activeCategory.label}</h3>
              <p className="text-xs text-gh-fg-muted">{activeCategory.description}</p>
            </div>
          </div>
      ...
      <Modal
        isOpen={modal.open}
        onClose={closeModal}
        title={modal.editing ? `Edit ${activeCategory.label.replace(/s$/, '')}` : `Add ${activeCategory.label.replace(/s$/, '')}`}
        footer={
          <>
